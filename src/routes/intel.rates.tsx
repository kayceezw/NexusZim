...
const SEED_RATES: RateRow[] = [
  {
    id: "s1",
    category: "DJ Services (Standard)",
    rate_low: 150,
    rate_high: 350,
    unit: "per event",
    notes: "+10% vs last quarter",
    updated_at: "2026-06-01T00:00:00Z",
  },
  {
    id: "s2",
    category: "Photography (8 hrs)",
    rate_low: 400,
    rate_high: 900,
    unit: "per day",
    notes: "Stable",
    ...
    <div className="space-y-12">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between border-b border-gold/10 pb-8">
        <div>
          ...
          <p className="mt-2 font-body text-sm text-foreground/70 max-w-xl">
            ...
            {isSeeded && (
              <span className="ml-1 font-mono text-[9px] text-gold uppercase tracking-widest">
                (Seed data — add live rates via Admin → Intel)
              </span>
            )}
          </p>
        </div>
        <div className="flex items-center gap-3 bg-gold/5 border border-gold/20 px-4 py-2">
          ...
          <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-gold">
            Updated{" "}
            {new Date(lastUpdated).toLocaleDateString("en-ZW", { month: "long", year: "numeric" })}
          </span>
        </div>
      </div>
      ...
      <div className="border border-gold/20 bg-gold/5 p-8">
        ...
        <p className="mt-4 font-body text-sm text-foreground/70 leading-relaxed">
          These rates are based on verified contracts brokered via NexusZim and public market
          surveys. Elite tier providers may charge premium rates above the high-end index due to
          audited portfolios and NexusZim endorsement.
        </p>
      </div>
    </div>
  );
