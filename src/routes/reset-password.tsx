  ...
  return (
    <div className="bg-background pt-16 min-h-screen grid place-items-center">
      <div className="w-full max-w-md my-10 px-5 sm:px-0">
        ...
        <div className="bg-card border border-border rounded-[6px] p-7 md:p-9">
          {pageState === "loading" && (
            <div className="py-16 flex flex-col items-center gap-5">
              <div className="h-7 w-7 border-2 border-border border-t-forest rounded-full animate-spin" />
          ...
          {pageState === "ready" && (
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              {error && (
                <div className="border border-rose-500/30 bg-rose-500/10 rounded-[3px] px-4 py-3 font-sans text-sm text-rose-600 dark:text-rose-400">
                  {error}
                </div>
              )}
                      ...
                      {strength.requirements.map((req) => (
                        <li
                          ...
                          className={`flex items-center gap-2 font-mono text-[9px] uppercase tracking-wider transition-colors ${
                            req.met ? "text-emerald-500" : "text-muted-foreground/30"
                          }`}
                        >
...
function getPasswordStrength(password: string) {
  ...
  const levels = [
    ...
    { label: "Secure", barColor: "bg-emerald-500", textColor: "text-emerald-600 dark:text-emerald-400" },
  ];
