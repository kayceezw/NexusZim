      ...
      <div className="container-page py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={s.label}
              className={`bg-card border rounded-[6px] p-5 ${
                s.highlight ? "border-amber-500/40 bg-amber-500/10" : "border-border"
              }`}
            >
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <select
              ...
              className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground border border-border rounded-[3px] bg-background px-3 py-2 focus:outline-none focus:border-primary"
            >
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="border border-border rounded-[6px] p-5 bg-card animate-pulse h-24" />
            ))}
          </div>
        ) : !items?.length ? (
          <div className="border border-dashed border-border rounded-[6px] p-14 text-center bg-card">
            <CheckCircle2 className="h-8 w-8 text-emerald-400 mx-auto mb-3" strokeWidth={1.5} />
            <p className="font-display text-xl text-foreground">Queue empty</p>
...
function SocialLinks({ rawData }: { rawData: QueueItem["raw_data"] }) {
  ...
  const PLATFORMS = [
    { key: "facebook",  Icon: Facebook,  color: "text-blue-600 dark:text-blue-400",  label: "Facebook" },
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="font-display text-lg text-foreground">{item.business_name}</p>
          <span
          ...
          {item.category_guess && (
            <span className="font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 border border-border text-muted-foreground rounded-[3px]">
      ...
      {item.status === "pending" && (
        <div className="flex flex-wrap gap-2 shrink-0">
          ...
          <button
            onClick={onDuplicate}
            disabled={isPending}
            className="flex items-center gap-1.5 border border-slate-500/40 text-slate-500 hover:bg-slate-500/10 px-4 py-2 rounded-[3px] font-mono text-[9px] uppercase tracking-widest transition-colors disabled:opacity-60"
          >
